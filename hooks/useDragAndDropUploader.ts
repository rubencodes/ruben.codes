...
function useDragAndDropUploader(
  dropAreaRef: React.MutableRefObject<Element | undefined>,
  handleFiles: (files: File[]) => void,
  handleFilesDependencies: any[],
) {
  ...
  useEffect(() => {
    ...
    function onDropFiles({ dataTransfer }: InputEvent) {
    ...
    dropArea?.addEventListener(
      EVENTS.Drop,
      onDropFiles as EventListener,
      false,
    );
    return () =>
      dropArea?.removeEventListener(EVENTS.Drop, onDropFiles as EventListener);
  }, [...handleFilesDependencies]);
